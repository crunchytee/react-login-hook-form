...
describe("LoginForm ", () => {
  ...
  describe("Does not validate ", () => {
    ...
    test("First Name when the input is one character long", async () => {
      ...
      const fNameError = screen.getByText(
    ...
    test("Last Name when the input is one character long", async () => {
      ...
      const lNameError = screen.getByText(
    ...
    test("Password when an invalid password is given", async () => {
      ...
      const pwError = screen.getByText(
  ...
  describe("After pressing the Submit button,", () => {
    // Re Route to the login form, fill it out valid, and submit it
    beforeEach(async () => {
      history.push(route);
      const firstName = screen.getByLabelText("First Name");
      const lastName = screen.getByLabelText("Last Name");
      ...
      const submitButton = screen.getByRole("button", { name: "Submit" });
      await act(async () => {
        fireEvent.input(firstName, { target: { value: "First" } });
        fireEvent.input(lastName, { target: { value: "Last" } });
        fireEvent.input(emailAddress, {
          target: { value: "Tosh@revrides.com" },
        });
        fireEvent.input(password, { target: { value: "12345678" } });
        fireEvent.input(confirmPassword, { target: { value: "12345678" } });
        fireEvent.input(phoneNumber, { target: { value: "5306872050" } });
        fireEvent.click(submitButton);
      });
    })

    // Test to see if first name is gone
    test("First name is gone", async () => {
      const firstNameQuery = screen.queryByText("First Name");
      expect(firstNameQuery).toBeNull;
    })

    test("Success text displays", async () => {
